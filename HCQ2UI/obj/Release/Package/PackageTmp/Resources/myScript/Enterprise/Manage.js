//var treeObj = Z_Tree_Help.Load(0, "unitTree", zTreeOnClick);
function zTreeOnClick(event, treeId, treeNode) {
    if (treeNode.type == 2) {
        $("#JianDie").val(treeNode.unit_id);
        tableHelp.refresh(table);
    }
};

var treeObj = Lutai.Load("seSheng", "seShi", "searchArea", "testTree", 0, zTreeOnClick);
...
var operion = {
    ...
    Save: function () {
        if ($("#unitAdd").valid()) {
            $.post($.GetIISName() + "/Enterprise/AddUnit", $("#unitAdd").serialize(), function (result) {
                if (result == "ok") {
                    TreeRefresh.Update();
                    tableHelp.refresh(table);
                    TextBoxOper.Null();
    ...
    EditSave: function () {
        if ($("#unitAdd").valid()) {
            $.ajax({
                type:"post",
                dataType:"text",
                data: $("#unitAdd").serialize(),
                url: $.GetIISName() + "/Enterprise/Edit",
        ...
        //   && unitname != null && unitname != "" && unitname != undefined
        //   && unittype != null && unittype != "" && unittype != undefined) {
    ...
    //删除
    Delete: function (row) {
        layer.confirm("确定要删除该单位或者部门吗？", { btn: ["确定", "取消"] }, function () {
            $.ajax({
                type: "post",
                dataType: "text",
                url: $.GetIISName() + "/Enterprise/Delete",
    ...
    SetValue: function (row) {
        ...
        if (row[0]["B0109"] != null && row[0]["B0109"] != "" && row[0]["B0109"] != undefined)
            $("#B0109").val(row[0]["B0109"].substring(0, 10));
        if (row[0]["B0110"] != null && row[0]["B0110"] != "" && row[0]["B0110"] != undefined)
            $("#B0110").val(row[0]["B0110"].substring(0, 10));
...
var FBDW = {
    Load: function () {
        $("#FBDW").click(function () {
            ...
            if (comUnit != null && comUnit != "" && comUnit != undefined) {
