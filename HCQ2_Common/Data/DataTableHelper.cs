...
namespace HCQ2_Common.Data
{
    ...
    public class DataTableHelper
    {
        ...
        /// <param name="isChangeDateToStr">是否需要将Table中的DateTime转为String</param>
        /// <returns>行中是对象的类，类的属性与数据字段一致</returns>
        public static List<T> DataTableToIList<T>(DataTable dt,bool isChangeDateToStr=false)
        {
            // 定义集合    
                ...
                foreach (PropertyInfo pi in propertys)
                {
                    ...
                    if (dt.Columns.Contains(tempName))
                    {
                        ...
                        if (value != DBNull.Value)
                            if (isChangeDateToStr && dr[tempName].GetType().Name.ToLower().Equals("datetime"))
                                pi.SetValue(obj, string.Format("{0:yyyy-MM-dd}", Convert.ToDateTime(value)), null);
                            else
