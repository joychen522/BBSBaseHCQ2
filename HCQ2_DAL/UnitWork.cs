...
{
    public class UnitWork: HCQ2_IDAL.IUnitWork
    {
        protected DbContext Context = HCQ2_DI.SpringHelper.GetObject<DBContextFactory>("DBContextFactory").GetDbContext();
        ...
        public IQueryable<T> Find<T>(int pageindex, int pagesize, string orderby = "", Expression<Func<T, bool>> exp = null) where T : class
        {
            ...
            return Filter(exp).ZJOrderBy(orderby).Skip<T>(pagesize * (pageindex - 1)).Take<T>(pagesize);
        }
